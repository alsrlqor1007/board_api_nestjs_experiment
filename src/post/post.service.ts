...
export class PostService {
  ...
  findAll(): Observable<AxiosResponse<any>> {
    return this.httpService.get('http://localhost:3000/post');
  };

  async createPost(postDto: PostDto): Promise<object> {
    const { writer_id, text } = postDto;
    const writerinfo = await this.userRepository.findOne({
  ...
  async getPostById(id): Promise<object> {
    ...
    if (!result) throw new NotFoundException('Not valid post id');
      ...
      const { post_id, writer_id, text, update_date_time, deleted_date_time } =
        result;
      const writer = (
        await this.userRepository.findOne({ where: { user_id: writer_id } })
      ).nickname;
  ...
  async modifyPost(patchDto: PatchDto) {
    const result = await this.postRepository.findOne({
      where: { post_id: patchDto.post_id },
    });
  ...
  async deletePost(post_id, writer_id) {
    ...
    if (!result) throw new NotFoundException('Not valid post id');
    else if (result.writer_id !== writer_id)
      throw new BadRequestException('Not the writer of the post');
    else {
